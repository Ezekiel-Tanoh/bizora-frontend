...
export default function Clients() {
  ...
  return (
    <>
      <style>{`
        .clients-header { display: flex; justify-content: space-between; align-items: center; margin-bottom: 2rem; }
      ...
      <div style={{ color: "#fff", fontFamily: "'Inter', sans-serif" }}>
        ...
        <NouveauClientModal isOpen={isModalOpen} onClose={() => setIsModalOpen(false)} onSuccess={fetchClients} />
        ...
        <div style={{ ...cardStyle, padding: 0, overflow: "hidden" }}>
          ...
          <div className="clients-table">
            <table style={{ width: "100%", borderCollapse: "collapse" }}>
              ...
              <tbody>
                {loading ? (
                  <tr><td colSpan={5} style={{ textAlign: "center", padding: "4rem", color: "rgba(255,255,255,0.3)" }}>Chargement...</td></tr>
                ) : filtered.length === 0 ? (
                  <tr>
                    ...
                    >
                      <td style={{ padding: "12px 16px" }}>
                        <div style={{ display: "flex", alignItems: "center", gap: "10px" }}>
                          <div style={{ width: "32px", height: "32px", borderRadius: "50%", background: "rgba(139,92,246,0.2)", display: "flex", alignItems: "center", justifyContent: "center", fontSize: "14px", fontWeight: "600", color: "#a78bfa" }}>
                            {client.name?.charAt(0).toUpperCase()}
                          </div>
                          <p style={{ fontSize: "14px", fontWeight: "500", margin: 0 }}>{client.name}</p>
                        </div>
                      </td>
                      <td style={{ padding: "12px 16px" }}><p style={{ fontSize: "13px", color: "rgba(255,255,255,0.6)", margin: 0 }}>{client.phone}</p></td>
                      <td style={{ padding: "12px 16px" }}><p style={{ fontSize: "13px", color: "rgba(255,255,255,0.4)", margin: 0 }}>{client.email || "—"}</p></td>
                      <td style={{ padding: "12px 16px" }}><p style={{ fontSize: "12px", color: "rgba(255,255,255,0.3)", margin: 0 }}>{new Date(client.createdAt).toLocaleDateString("fr-FR")}</p></td>
                      <td style={{ padding: "12px 16px" }}>
                        <div style={{ display: "flex", gap: "12px" }}>
                          <button style={{ fontSize: "12px", color: "#a78bfa", background: "none", border: "none", cursor: "pointer", padding: 0 }}>Modifier</button>
                          <button onClick={() => handleDelete(client.id)} style={{ fontSize: "12px", color: "#f87171", background: "none", border: "none", cursor: "pointer", padding: 0 }}>Supprimer</button>
                        </div>
                      </td>
                    </tr>
                  ))
          ...
          <div className="clients-cards">
            {loading ? (
              ...
              <div style={{ textAlign: "center", padding: "2rem 0" }}>
                ...
                <p style={{ fontSize: "14px", color: "rgba(255,255,255,0.3)" }}>Aucun client</p>
              </div>
            ) : (
              filtered.map((client) => (
                <div key={client.id} style={{ background: "rgba(255,255,255,0.03)", border: "1px solid rgba(255,255,255,0.07)", borderRadius: "12px", padding: "1rem" }}>
                  <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: "8px" }}>
                    <div style={{ display: "flex", alignItems: "center", gap: "10px" }}>
                      <div style={{ width: "36px", height: "36px", borderRadius: "50%", background: "rgba(139,92,246,0.2)", display: "flex", alignItems: "center", justifyContent: "center", fontSize: "15px", fontWeight: "600", color: "#a78bfa" }}>
                        {client.name?.charAt(0).toUpperCase()}
                      </div>
                      <div>
                        <p style={{ fontSize: "14px", fontWeight: "600", margin: "0 0 2px" }}>{client.name}</p>
                        <p style={{ fontSize: "12px", color: "rgba(255,255,255,0.4)", margin: 0 }}>{client.phone}</p>
                      </div>
                    </div>
                    <div style={{ display: "flex", gap: "8px" }}>
                      <button style={{ fontSize: "12px", color: "#a78bfa", background: "none", border: "none", cursor: "pointer" }}>Modifier</button>
                      <button onClick={() => handleDelete(client.id)} style={{ fontSize: "12px", color: "#f87171", background: "none", border: "none", cursor: "pointer" }}>Suppr.</button>
                    </div>
                  </div>
                  {client.email && <p style={{ fontSize: "12px", color: "rgba(255,255,255,0.3)", margin: 0 }}>{client.email}</p>}
                </div>
              ))
            )}
          </div>
