      ...
      <div style={{ color: "#fff", fontFamily: "'Inter', sans-serif" }}>
        ...
        <div className="stock-header">
          <div>
            ...
            <p style={{ fontSize: "14px", color: "rgba(255,255,255,0.4)", margin: "4px 0 0" }}>Gérez votre inventaire</p>
          </div>
            ...
            {produits.filter(p => p.stock > 0 && p.stock <= 5).length === 0 ? (
              ...
              produits.filter(p => p.stock > 0 && p.stock <= 5).map(p => (
                <div key={p.id} style={{ display: "flex", justifyContent: "space-between", padding: "8px 0", borderBottom: "1px solid rgba(251,191,36,0.1)" }}>
                  ...
                  <span style={{ fontSize: "12px", padding: "2px 8px", borderRadius: "100px", background: "rgba(251,191,36,0.1)", color: "#fbbf24" }}>{p.stock} restants</span>
                </div>
              ))
            )}
          </div>
            ...
            {produits.filter(p => p.stock === 0).length === 0 ? (
              ...
              produits.filter(p => p.stock === 0).map(p => (
                <div key={p.id} style={{ display: "flex", justifyContent: "space-between", padding: "8px 0", borderBottom: "1px solid rgba(239,68,68,0.1)" }}>
                  <p style={{ fontSize: "13px", color: "rgba(255,255,255,0.7)", margin: 0 }}>{p.name}</p>
                  <span style={{ fontSize: "12px", padding: "2px 8px", borderRadius: "100px", background: "rgba(239,68,68,0.1)", color: "#f87171" }}>Rupture</span>
                </div>
              ))
            )}
          </div>
              ...
              <tbody>
                {loading ? (
                  ...
                  produits.map((produit) => (
                    ...
                    >
                      <td style={{ padding: "12px 16px" }}><p style={{ fontSize: "14px", fontWeight: "500", margin: 0 }}>{produit.name}</p></td>
                      ...
                      <td style={{ padding: "12px 16px" }}><p style={{ fontSize: "14px", fontWeight: "600", margin: 0 }}>{produit.stock}</p></td>
                      <td style={{ padding: "12px 16px" }}>
                        {produit.stock === 0 ? (
                      ...
                      <td style={{ padding: "12px 16px" }}><p style={{ fontSize: "14px", color: "#a78bfa", fontWeight: "600", margin: 0 }}>{produit.price?.toLocaleString()} FCFA</p></td>
                    </tr>
                  ))
                )}
              </tbody>
            ...
            {loading ? (
              ...
              produits.map((produit) => (
                <div key={produit.id} style={{ background: "rgba(255,255,255,0.03)", border: "1px solid rgba(255,255,255,0.07)", borderRadius: "12px", padding: "1rem" }}>
                  ...
                  <div style={{ display: "flex", justifyContent: "space-between" }}>
                    <p style={{ fontSize: "13px", color: "#a78bfa", fontWeight: "600", margin: 0 }}>{produit.price?.toLocaleString()} FCFA</p>
                    <p style={{ fontSize: "13px", color: "rgba(255,255,255,0.4)", margin: 0 }}>Stock: {produit.stock}</p>
                  </div>
                </div>
              ))
            )}
